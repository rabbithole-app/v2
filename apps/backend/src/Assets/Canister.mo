...
shared ({ caller = owner }) actor class AssetsCanister(canister_args : T.CanisterArgs) = this_canister {
  ...
  public shared query ({ caller = _ }) func get(args : T.GetArgs) : async T.EncodedAsset {
  ...
  public shared query ({ caller = _ }) func get_chunk(args : T.GetChunkArgs) : async (T.ChunkContent) {
  ...
  public shared query ({ caller = _ }) func list(args : {}) : async [T.AssetDetails] {
    assets.list(args);
  };
  ...
  public shared ({ caller = _ }) func list_permitted(args : T.ListPermitted) : async ([Principal]) {
  ...
  public shared ({ caller = _ }) func certified_tree({}) : async (T.CertifiedTree) {
    let res = assets.certified_tree();
    await* Utils.throw_if_error(res);
    Utils.extract_result(res);
  };
  public shared ({ caller = _ }) func validate_grant_permission(args : T.GrantPermission) : async (Result<Text, Text>) {
    assets.validate_grant_permission(args);
  };

  public shared ({ caller = _ }) func validate_revoke_permission(args : T.RevokePermission) : async (Result<Text, Text>) {
    assets.validate_revoke_permission(args);
  };

  public shared ({ caller = _ }) func validate_take_ownership() : async (Result<Text, Text>) {
    assets.validate_take_ownership();
  };

  public shared ({ caller = _ }) func validate_commit_proposed_batch(args : T.CommitProposedBatchArguments) : async (Result<Text, Text>) {
    assets.validate_commit_proposed_batch(args);
  };

  public shared ({ caller = _ }) func validate_configure(args : T.ConfigureArguments) : async (Result<Text, Text>) {
