...
# Файлы шифруются до того, как покинут ваш браузер

Когда вы загружаете файл в Rabbithole, он никогда не покидает устройство в открытом виде. Шифрование происходит полностью в браузере — ваша канистра видит только запечатанные данные.
...
- **Аппаратная изоляция на подходе** — Internet Computer [развивает поддержку TEE](https://forum.dfinity.org/t/upcoming-proposal-the-first-tee-enabled-subnet/64180) на базе AMD SEV-SNP, которая добавит аппаратную изоляцию памяти канистры — даже операторы узлов не смогут получить доступ к данным в памяти. Для Rabbithole это означает двойную защиту: шифрование vetKeys + аппаратная изоляция
- **Если Rabbithole закроется** — ваши данные останутся на блокчейне
