...
Rabbithole не нужен email, чтобы понять, какой аккаунт вошёл в приложение.
Для этого используется principal-идентификатор Internet Computer.
...
principal-идентификатором и разрешениями, сохранёнными в вашем хранилище.
...
- Канистру приложения Rabbithole
...
- При прямом входе через `<ваше-хранилище>.icp.net` Internet Identity выводит
...
Internet Identity и отправляется в Rabbithole так, что канистра приложения
может его проверить.
...
identity, в которую добавлены эти атрибуты. Канистра приложения проверяет
подпись, nonce, origin, время и caller, а затем сохраняет проверенные атрибуты
для principal-идентификатора. Получение доступа к storage invite остаётся
отдельным шагом и использует только уже верифицированный email.
...
sequenceDiagram
    ...
    participant API as Канистра приложения Rabbithole
...
Канистра приложения отклоняет устаревшие, неподписанные, повторно использованные,
