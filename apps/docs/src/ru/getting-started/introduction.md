---
title: Введение
description: Суверенное зашифрованное хранилище на Internet Computer
---

# Rabbithole — суверенное зашифрованное хранилище на Internet Computer

Rabbithole — суверенное зашифрованное хранилище для тех, кому нужен привычный
облачный диск, но без центрального владельца, которому приходится доверять
файлы, права доступа и ключи шифрования.

В обычном облаке ваши файлы, логика приложения, интерфейс и правила доступа
...
приложение может работать целиком: фронтенд, логика приложения, состояние и
пользовательские хранилища не нужно выносить в обычное облако. Благодаря этому
Rabbithole создаёт для вас не просто запись в базе данных, а отдельное
хранилище, реализованное как канистра Internet Computer, и передаёт контроль
над ним вам.
...
Файл шифруется прямо в браузере, ещё до загрузки. Открытые данные не
отправляются в канистру хранилища, а ключи не выводятся из пароля и не
хранятся как мастер-ключ у Rabbithole. Технические страницы позже объясняют
...
заложены в архитектуру, а не держатся только на обещании сервиса.
...
1. Вы входите через [Internet Identity](https://id.ai), без отдельного
   пароля Rabbithole.
2. Rabbithole создаёт независимое хранилище для вашего аккаунта.
3. Вы загружаете файлы через приложение.
4. Браузер шифрует файл до загрузки.
...
защитит свою инфраструктуру, правильно применит права доступа, сохранит сервис
...
живут вместе с вашим хранилищем. Файлы шифруются в браузере, а ключи
выводятся сетью, а не из мастер-ключа, который Rabbithole хранит у себя.
...
Открытый текст не загружается в канистру или Blob Storage. Rabbithole всё равно
полагается на ваш браузер, консенсус IC и корректный код. Эти допущения
перечислены на странице [Модель доверия](/ru/how-it-works/trust-model).
