...
passes verification in the Rabbithole application canister.
...
it only after the application canister verifies the identity payload.

The application canister checks these facts before using the email:
...
The storage canister accepts application-canister attestation only from the
trusted Rabbithole application canister configured for that storage. A direct
storage app session can also claim matching email grants after it receives
verified attributes from the identity flow.
