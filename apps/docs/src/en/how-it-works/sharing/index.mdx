...
Shared access is part of [Pro](/how-it-works/pro). Starter Vault keeps
the owner's personal encrypted storage available within issued limits, but it
does not replace active Pro for invites, access management, or invited
users.
...
:::tip Why email invites matter

You can share access with a future user. When that person later signs in with
the same verified email from
[Identity Attributes](/how-it-works/authentication#trust-verified-attributes), Rabbithole connects
...
:::details Technical view: canisters, events, and the Shared with me list

The storage canister makes the access decision. The Rabbithole application
canister keeps a synchronized index so users can discover storages shared with
them in the main app.
...
If the application-canister index is unavailable, the storage canister still
owns the permission decision. The index improves discovery and notifications;
it doesn't replace canister checks.
...
- **Verified email**: an
  [Identity Attribute](/how-it-works/authentication#trust-verified-attributes) proved by the
  sign-in flow, not a free-form text field.
- **Grant**: an access rule stored by the storage canister.
- **Active Pro**: the state required for shared access and access
  management.
    ...
    items: [
      {
        ...
        link: '/how-it-works/sharing/email-invites',
      },
      {
        text: 'Permission model',
        link: '/how-it-works/sharing/permissions',
      },
      {
        text: 'Encrypted sharing',
        link: '/how-it-works/sharing/encryption-in-shared-access',
      },
      {
        text: 'Authentication and identity attributes',
        link: '/how-it-works/authentication#trust-verified-attributes',
      },
      {
