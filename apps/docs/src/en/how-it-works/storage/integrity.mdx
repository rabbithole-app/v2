...
If a file is silently replaced on the way to you, Rabbithole should refuse to open it.  
...
For Blob Storage, Rabbithole certifies the metadata that tells the browser what file it should expect.
...
- if encryption is enabled, decryption only happens after integrity passes
