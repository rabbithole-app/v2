...
By default the bytes are held by [**Caffeine Blob Storage**](https://caffeine.ai/) —
an external service that keeps them on S3-compatible storage and handles paying
for the space and keeping the stored data in order. With Pro you can connect
[your own S3 bucket](/how-it-works/storage/own-s3-storage) instead and pay your
provider directly; the flow is the same either way.
...
- **Caffeine Blob Storage** holds the encrypted bytes. Inside it, a **gateway**
  accepts uploads and returns files during download, a **cashier** pays for the
  storage, and a **cleanup** part keeps the stored data in sync with your
  canister.
...
and retention rules. Caffeine Blob Storage handles this for you:

- its **gateway** accepts uploads and returns files during download;
- its **cashier** keeps the storage funded;
- its **cleanup** part synchronizes deletion and retention events with your
  canister.
