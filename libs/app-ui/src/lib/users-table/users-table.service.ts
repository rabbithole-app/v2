...
function convertProfile(item: ProfileRaw): Profile {
  return {
    ...
    displayName: isNonNullish(item.displayName)
      ? item.displayName[0]
      : undefined,
    avatarUrl: isNonNullish(item.avatarUrl)
      ? item.avatarUrl[0]
      : undefined,
...
export class UsersTableService {
  ...
  readonly listOptions = computed(() => {
    ...
    const options: ListOptions = {
      ...
      count: true,
      sort: [[sorting.field, sorting.direction === 'asc' ? { Ascending: null } : { Descending: null }]],
      filter: {
        id: [],
        ...
        createdAt: filters.dateFilter ? [{
          min: filters.dateFilter.min ? [toBigIntNanoSeconds(filters.dateFilter.min)] : [],
          max: filters.dateFilter.max ? [toBigIntNanoSeconds(filters.dateFilter.max)] : [],
        }] : [],
        avatarUrl: [],
      },
    };
  ...
  async loadProfiles(actor: RabbitholeActorService) {
    ...
    try {
      ...
      this.setError(error instanceof Error ? error.message : 'Failed to load profiles');
    } finally {
  ...
  setData(data: Profile[], totalCount: number) {
    this._state.update(state => ({ ...state, data, totalCount, error: null }));
  }
