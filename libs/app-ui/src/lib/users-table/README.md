...
```typescript
import { UsersTableComponent } from '@rabbithole/app-ui';

@Component({
  selector: 'app-users',
  standalone: true,
  imports: [UsersTableComponent],
  template: '<rbth-users-table [data]="users" [totalCount]="totalCount" [loading]="isLoading" (pageChange)="onPage($event)" />'
})
...
```typescript
import { UsersTableComponent, UsersTableService } from '@rabbithole/app-ui';
